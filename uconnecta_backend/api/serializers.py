...
class RegisterSerializer(serializers.Serializer):
    ...
    how_to_address = serializers.CharField(max_length=30)
...
class MeSerializer(serializers.ModelSerializer):
    how_to_address = serializers.CharField(source="profile.how_to_address", read_only=True)
    ...
    class Meta:
        model = User
        fields = ["id", "email", "phone", "username", "how_to_address",
                  "profile", "cars", "isPremium", "rating"]
...
class MeUpdateSerializer(serializers.Serializer):
    ...
    surname = serializers.CharField(max_length=50, required=False, allow_null=True, allow_blank=True)
    patronymic = serializers.CharField(max_length=50, required=False, allow_null=True, allow_blank=True)
    how_to_address = serializers.CharField(max_length=30, required=False, allow_null=True, allow_blank=True)
    about = serializers.CharField(required=False, allow_null=True, allow_blank=True)
    settings = serializers.JSONField(required=False)

    def validate_email(self, v):
        if User.objects.filter(email=v).exclude(pk=self.context["request"].user.pk).exists():
            raise serializers.ValidationError("Email already exists")
        return v

    def validate_phone(self, v):
        if User.objects.filter(phone=v).exclude(pk=self.context["request"].user.pk).exists():
            raise serializers.ValidationError("Phone already exists")
        return v

    def validate_username(self, v):
        if User.objects.filter(username=v).exclude(pk=self.context["request"].user.pk).exists():
    ...
    def update(self, instance, validated_data):
        ...
        for f in ["name", "surname", "patronymic", "how_to_address", "about", "settings"]:
            if f in validated_data:
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Message
        fields = ["id", "chat", "sender", "sender_username", "body", "image", "created_at"]
...
class CallSerializer(serializers.ModelSerializer):
    class Meta:
        model = Call
        fields = ["id", "caller", "receiver", "started_at", "ended_at", "status", "call_token"]
